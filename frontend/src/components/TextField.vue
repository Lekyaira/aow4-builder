...
<template>
  <div class="flex flex-col gap-1 text-sm">
    <label :for="inputId" class="font-medium">
      <span v-if="required && label" aria-hidden="true" class="text-red-600"
        >*</span
      >
      {{ label }}
    </label>
    <div class="block relative w-full">
      <input
        :type="password ? (password_visible ? 'text' : 'password') : 'text'"
        :id="inputId"
        ...
        class="mt-1 w-full bg-white dark:bg-slate-800 border border-parchment-edge/70 dark:border-slate-700 rounded-herald px-4 py-3 font-display text-2xl sm:text-3xl tracking-wide shadow-sm focus:outline-none focus:ring-2 focus:ring-aspect-order/70 disabled:opacity-50"
      />
